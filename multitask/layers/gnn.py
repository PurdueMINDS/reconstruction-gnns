...
from torch_geometric.nn import global_add_pool, global_mean_pool, global_max_pool, PNAConv, BatchNorm, Set2Set
...
class NetPNA(torch.nn.Module):
    def __init__(self, node_size, edge_size, out_size, deg):
        ...
        self.set2set = Set2Set(1 * hidden_size, processing_steps=4)

        if out_size is None:
            self.mlp = Sequential(Linear(2*hidden_size, hidden_size), ReLU())
        else:
            self.mlp = Sequential(Linear(2*hidden_size, hidden_size), ReLU(), Linear(hidden_size, hidden_size//2), ReLU(),  Linear(hidden_size//2, out_size))
...
class NetSubgraphPNA(torch.nn.Module):
    def __init__(self, node_size, edge_size, out_size, deg):
        ...
        self.gnn = NetPNA(node_size, edge_size, None, deg)
