...
""" Run: $ srun -n 1 --gres=gpu:1 --cpus-per-task=1 --nice=1000 --partition=copa-gpu --output=gine.txt python -u gine.py & """
...
print("# of nodes", sum([ data.x.size(0) for data in dataset])/len(dataset) )
print("# of edge", sum([ data.edge_index.size(1)/2 for data in dataset])/len(dataset) )
sys.exit()
...
for _ in range(5):
    ...
    model = NetGINE( node_size, edge_size, hidden_size, 3 )
