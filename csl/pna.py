...
for train, val, test in zip(train_idx, val_idx, test_idx):
    ...
    model = NetPNA( node_size, edge_size, dataset.num_classes, deg )
    ...
    def train():
        ...
        for i in range(0, len(train_dataset), batch_size):
            ...
            loss = lf(model(data.x.long(), data.edge_index, data.edge_attr.long(), data.batch), data.y)
            loss.backward()
    ...
    def test(dataset, model):
        ...
        for i in range(0, len(dataset), 256):
            ...
            data = Batch().from_data_list(data).to(device)
            output = model(data.x.long(), data.edge_index, data.edge_attr.long(), data.batch)
