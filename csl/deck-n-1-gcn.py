...
""" Run: $ srun -n 1 --gres=gpu:1 --cpus-per-task=1 --nice=1000 --partition=copa-gpu --output=deck1gcn.txt python -u deck-n-1-gcn.py & """
...
for train, val, test in zip(train_idx, val_idx, test_idx):
    ...
    def train():
        ...
        for i in range(0, len(train_dataset), batch_size):
            ...
            loss = lf( model(subgraph_data.x_.double(), subgraph_data.edge_index, subgraph_data.edge_attr_.double(), subgraph_data.batch, weights, subgraph_batch), y )
            loss.backward()
