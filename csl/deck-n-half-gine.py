...
""" Run: $ srun -n 1 --gres=gpu:1 --cpus-per-task=1 --nice=1000 --partition=ml-all-gpu --output=deck-half.txt python -u deck-n-half-gine.py & """
...
for train, val, test in zip(train_idx, val_idx, test_idx):
    ...
    model = NetSubgraphGINE( node_size, edge_size, hidden_size, dataset.num_classes)
